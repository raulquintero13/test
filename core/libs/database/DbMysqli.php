...
class DbMysqli
{
    ...
    public function log_db_errors( $error, $query )
    {
        ...
        if( defined( 'SEND_ERRORS_TO' ) )
        {
            ...
            $headers .= 'From: Yoursite <system@montesito'.$_SERVER['SERVER_NAME'].'.com>' . "\r\n";
    ...
    public function __construct($dbConfig)
    {
        ...
        try {
            $this->link = new \mysqli( $dbConfig['host'], $dbConfig['user'], $dbConfig['password'], $dbConfig['dbname'] );

            $this->link->set_charset( "utf8" );
        } catch ( Exception $e ) {
            //header("Location: /missingdatabase.html");
            die ("no se pudo conectar a la base de datos: ".$dbConfig['dbname']. " <br>".$e);
        }
    }
