...
//  exec('tail ./test_error.log', $error_logs);
//  $error_logs = array_reverse($error_logs);
// echo "<table cellpadding=10>";
//   foreach($error_logs as $error_log) {
//         $error = explode("] ",$error_log);
//         $error[4]=str_replace('\n', '**', $error[4]);
//         $error_msg = explode("**",$error[4]);
//         // var_dump($error_msg);
//        echo "<tr><td>".$str = ltrim($error[0], '[')."</td><td>".ltrim($error[1],'[:')."<br>".ltrim($error[3],'[client')."</td><td valign=top><b>".$error_msg[0]."</b><br>".$error_msg[3]."<br>".$error_msg[5]."</td></tr>";
//   }
//   echo "</table>";
...
function get_log_files($dir, &$results = array()) {
  ...
  // $dir_list = array();
	if($files){
      ...
      $path1= explode("**",$path1);
      $name = $path1[count($path1)-1];
			$results[$dir][$name] = array('name' => $name, 'path' => $path);
      // print_r($results[$dir][$name]);die;
    }
    if(isset($dir_list)) 
...
function show_list_of_files($files, $lines  = 50){
	...
	foreach ($files as $dir => $files_array) {
		...
		foreach($files_array as $k=>$f){
			...
			$active = 0; //($f['path'] == $log) ? 'class="pure-menu-selected"':'';
